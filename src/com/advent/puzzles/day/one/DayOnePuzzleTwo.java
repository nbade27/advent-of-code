package com.advent.puzzles.day.one;
...
public class DayOnePuzzleTwo {
    public static void main(String[] args) {
        ...
        String input = "dssmtmrkonedbbhdhjbf9hq\n" +
        ...
        // extracting text from calibration document
        for (int j = 0; j < arr.length; j++) {
            String string = arr[j];
            System.out.println("working for the string :" + string);
            List<Integer> numbersList = new ArrayList<>();


            for (int start = 0; start < string.length(); start++) {
                if (isNumber(string.charAt(start))){
                    numbersList.add(Integer.parseInt(String.valueOf(string.charAt(start))));
                }
                for (int end = start; end < string.length(); end++) {
                    if (isNumber(string.charAt(end)) && start == end){
                        String subString = string.substring(start, end);
                        if(numbersMap.get(subString) != null){
                            numbersList.add(numbersMap.get(subString));
                        }
                        numbersList.add(Integer.parseInt(String.valueOf(string.charAt(end))));
                    }

                    String subString = string.substring(start, end);
                    if(numbersMap.get(subString) != null){
                        numbersList.add(numbersMap.get(subString));
                    }else if(end == string.length()-1){
                        String subStringLast = string.substring(start, end+1);
                        if(numbersMap.get(subStringLast) != null){
                            numbersList.add(numbersMap.get(subStringLast));
                            start = end;
                        }
                    }
            ...
            String finalNumberString = numbersList.get(0) + "" + numbersList.get(numbersList.size() -1);
            int finalNum = Integer.parseInt(finalNumberString);
